function DG_read_Intan_RHD2000_file(filename)
...
if (data_present)
    ...
    time = zeros(1, num_amplifier_samples);
    ...
    for i=1:num_data_blocks
        ...
        || (data_file_main_version_number > 1))
            time(amplifier_index:(amplifier_index+59)) = fread(fid, 60, 'int32');
        else
            time(amplifier_index:(amplifier_index+59)) = fread(fid, 60, 'uint32');
        end
...
if (data_present)
    ...
    t_aux_input = time(1:4:end);
    t_supply_voltage = time(1:60:end);
    t_board_adc = time;
    t_dig = time;
...
if (data_present)
    ...
    move_to_base_workspace(time);
    sampleRate = sample_rate;
    move_to_base_workspace(sampleRate);
end
